variable "name" {
  type        = string
  description = "Name of the PaaS, usually same as customer name"
}
...
variable "dih_oracle_helm_sets" {
  ...
  default = {
    ...
    #   "diOracleDB.resources.requests.ephemeral-storage" : "20Gi"
    #   "diOracleDB.resources.limits.ephemeral-storage" : "20Gi"
  }
}
...
variable "bastion_user_data" {
  ...
  default     = <<-EOT
    ...
    # sudo su - root -c "curl -fsSL https://raw.githubusercontent.com/firezone/firezone/legacy/docker-compose.prod.yml -o docker-compose.yml"
    # PUBLIC_IP=`curl -s http://169.254.169.254/latest/meta-data/public-ipv4`
    # sudo su - root -c "docker run --rm firezone/firezone bin/gen-env > .env"
    # sudo sed -i -E "s/^EXTERNAL_URL=.*/EXTERNAL_URL=https:\/\/$PUBLIC_IP/g" ~root/.env
    # sudo sed -i -E "s/^DEFAULT_ADMIN_EMAIL=.*/DEFAULT_ADMIN_EMAIL=devops@gigaspaces.com/g" ~root/.env

    # sudo su - root -c "docker-compose run --rm firezone bin/migrate"
    # sudo su - root -c "docker-compose run --rm firezone bin/create-or-reset-admin"
