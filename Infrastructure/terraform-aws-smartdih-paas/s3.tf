...
locals {
  bucket_names = {
    data    = "gs-paas-${replace(var.name, "/\\W|_|\\s/", "-")}-data-${try(random_string.s3suffix[0].id, "")}"
    control = "gs-paas-${replace(var.name, "/\\W|_|\\s/", "-")}-control-${try(random_string.s3suffix[0].id, "")}"
  }

  s3_buckets_policy = "${replace(var.name, "/\\W|_|\\s/", "-")}-paas-s3-policy"
  s3_username       = "${replace(var.name, "/\\W|_|\\s/", "-")}-paas-s3-user"
}
