...
locals {
  ...
  opensearch_policy = "${replace(var.name, "/\\W|_|\\s/", "-")}-paas-aoss-policy"
  extra_iam_roles = [
...
module "opensearch-logging-policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      #   Resource: "arn:aws:aoss:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:collection/${aws_opensearchserverless_collection.collection.id}"
      # },
      {
        Sid : "AOSSDashboards",
        Effect : "Allow",
