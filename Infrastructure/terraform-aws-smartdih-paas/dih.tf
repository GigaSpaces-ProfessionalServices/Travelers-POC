locals {
  ingress_host = "${local.name}.${trim(data.aws_route53_zone.paas.name, ".")}"

  grafana_ingress_name = "${local.name}-grafana"
  grafana_ingress_host = "${local.name}-grafana.${trim(data.aws_route53_zone.paas.name, ".")}"
